...
$out = dirname(__DIR__) . '/uploads/staff_contracts/generated/_fresh.docx';
@unlink($out);
...
$bad = [];
foreach (['${', 'employer_name', 'employer_date', 'full_name', 'signing_date', 'employer_signature', 'employee_signature'] as $n) {
    if (strpos($x, $n) !== false) $bad[] = $n;
}
echo "\n" . ($bad ? 'FAIL: '.implode(', ',$bad) : 'ALL OK') . "\n";
