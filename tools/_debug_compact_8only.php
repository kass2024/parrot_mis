...
for ($i = 0; $i < 8; $i++) {
    $xml = preg_replace($pattern, '<w:r><w:br w:type="page"/></w:r></w:p>', $xml, 1) ?? $xml;
}
echo 'after 8 one-by-one: len=' . strlen($xml) . ' hard=' . substr_count($xml, 'w:type="page"') . "\n";
libxml_use_internal_errors(true);
echo simplexml_load_string($xml) ? "valid\n" : "INVALID\n";
echo 'more matches=' . preg_match_all($pattern, $xml) . "\n";
