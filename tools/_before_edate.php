...
foreach (pcvc_staff_contract_placeholder_keys() as $key) {
    if ($key === 'employer_date') {
        ...
        echo "BEFORE employer_date key:\n";
        echo (strpos(substr($xml,$p,12000), 'Date:Parrott') !== false ? 'HAS Date:Parrott' : 'no Date:Parrott') . "\n";
        echo (strpos($xml, 'employer_date') !== false ? 'has employer_date tag' : 'no employer_date tag') . "\n";
        break;
    }
