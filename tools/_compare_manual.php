...
echo (strpos(substr($xml,$p,12000), 'Date:Parrott') !== false ? 'HAS Date:Parrott' : 'no') . "\n";

$z2 = new ZipArchive();
$z2->open(dirname(__DIR__) . '/admin/Parrot Contract for Mutware.docx');
$xml2 = (string) $z2->getFromName('word/document.xml');
$z2->close();
$merged = pcvc_staff_contract_apply_placeholder_values($xml2, $values, $imageKeys);
$p2 = strpos($merged, 'ACCEPTANCE');
echo 'apply_placeholder: ' . (strpos(substr($merged,$p2,12000), 'Date:Parrott') !== false ? 'HAS Date:Parrott' : 'no') . "\n";
