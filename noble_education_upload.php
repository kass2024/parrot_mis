...
/**
 * Noble Education Group — Canada live file upload.
 */
...
try {
    ...
    if (!in_array($field, ['passport', 'high_school_certificate', 'transcripts', 'other'], true)) {
        $field = 'file';
    }
    ...
    $permDir = __DIR__ . '/uploads/noble_education';
    if (!is_dir($permDir) && !mkdir($permDir, 0755, true) && !is_dir($permDir)) {
        throw new RuntimeException('Cannot create upload directory');
    }

    $filename = 'neg_' . date('Ymd') . '_' . bin2hex(random_bytes(8)) . '_' . $field . '.' . $ext;
    ...
    echo json_encode([
        ...
        'file_path' => 'uploads/noble_education/' . $filename,
